...
from scapy.all import IP, UDP, TCP, Ether, get_if_hwaddr, get_if_addr, get_if_list, sendp, AsyncSniffer, sniff, bytes_hex, hex_bytes, Packet, raw
from ltp import *

UDP_PROTOCOL = 1
TCP_PROTOCOL = 2
PORT_NUMER = 1
SWITCH_INTERFACE = "s2-eth" + str(PORT_NUMER)
...
def handle_pkt(packet, tx_function, iface):       
    if IP in packet:
        ...
        if "10.0.2.1" == packet[IP].dst:
            return
    else:
        packet = LTP(raw(packet))
        if 1 == packet[LTP].dev_id:
            return
